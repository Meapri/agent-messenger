...
# List only chat rooms with unread messages
agent-kakaotalk chat unread
agent-kakaotalk chat unread --resolve-titles
```
...
# Search recent messages in one chat
agent-kakaotalk message search <chat-id> "query"
agent-kakaotalk message search <chat-id> "삼성전자|SK하이닉스" --regex

# Export recent messages
agent-kakaotalk message export <chat-id> --format json
agent-kakaotalk message export <chat-id> --format jsonl -o ./chat.jsonl
agent-kakaotalk message export <chat-id> --format csv -n 1000
...
agent-kakaotalk message react <chat-id> <log-id>
agent-kakaotalk message react <chat-id> <log-id> 1

# Download a media attachment from a message
agent-kakaotalk message download <chat-id> <log-id>
agent-kakaotalk message download <chat-id> <log-id> -o ./photo.jpg

# Edit a message by log ID (experimental; some device slots return -203)
agent-kakaotalk message edit <chat-id> <log-id> "Edited text"

# Delete a message by log ID
agent-kakaotalk message delete <chat-id> <log-id>
...
#### Search, Export, Download, Edit

`message search` filters the recent message window from `message list`; increase `-n` when you need a wider scan. It supports plain substring matching by default and `--regex` for JavaScript regular expressions.

`message export` writes the same message shape as `message list` in `json`, `jsonl`, `csv`, or `txt`. Use `-o <path>` to save to a file.

`message download` resolves a message by `log_id`, finds attachment URL fields such as `url`, `downloadUrl`, `imageUrls`, or `thumbnailUrls`, and writes the downloaded bytes to disk. By default it only downloads `https://*.kakao.com` and `https://*.kakaocdn.net` URLs. Use `--url-key`, `--url-index`, or `--allow-external-url` only when inspecting a known attachment shape.

`message edit` sends LOCO `REWRITE`. KakaoTalk may reject this for some desktop/sub-device profiles with a non-zero status such as `-203`; the CLI returns `{ "success": false }` and exits non-zero in that case.
...
- Message reactions are exposed as numeric KakaoTalk reaction types. Type `1` is the broadly verified default; newer reaction IDs are accepted by the CLI/SDK but may depend on the KakaoTalk device slot and server support.
- Message editing is exposed through `message edit`, but KakaoTalk can reject `REWRITE` for some desktop/sub-device profiles.
- No open chat browsing or joining
- No search across all chats in one command (`message search` searches one chat at a time)
