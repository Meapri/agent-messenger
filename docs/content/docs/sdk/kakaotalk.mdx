...
// List chat rooms that currently have unread messages
const unread = await client.getUnreadChats({ resolveTitles: true })
```
...
// Search within one chat's recent message window
const hits = await client.searchMessages('9876543210', 'release', { count: 200 })
const regexHits = await client.searchMessages('9876543210', '\\bAPR\\b', { regex: true })

// Download a media attachment by log ID (with writeFile from node:fs/promises)
const download = await client.downloadAttachment('9876543210', '3846830417126748160')
await writeFile(download.filename, Buffer.from(download.data))
```
...
// Add a reaction to an existing message. Type 1 is the broadly verified
// KakaoTalk default; pass newer numeric types as KakaoTalk exposes them.
const reaction = await client.reactMessage('9876543210', '3846830417126748160')
// → KakaoReactionResult { success, status_code, chat_id, log_id, reaction_type }

// Delete a message by log ID
const deleted = await client.deleteMessage('9876543210', '3846830417126748160')
// → KakaoDeleteMessageResult { success, status_code, chat_id, log_id }

// Edit a message by log ID. Some KakaoTalk desktop/sub-device profiles reject
// REWRITE (commonly status -203), which returns success=false instead of throw.
const edited = await client.editMessage('9876543210', '3846830417126748160', 'Edited text')
// → KakaoEditMessageResult { success, status_code, chat_id, log_id, message }
```
...
listener.on('reaction', (event) => {
  console.log(`User ${event.user_id} reacted to ${event.log_id} with type ${event.reaction_type}`)
})

listener.on('message_deleted', (event) => {
  console.log(`Message ${event.log_id} was deleted in ${event.chat_id}`)
})

listener.on('message_edited', (event) => {
  console.log(`Message ${event.log_id} was edited to: ${event.message}`)
})
...
| `reaction`        | `KakaoTalkPushReactionEvent`       | Message reaction changed                |
| `message_deleted` | `KakaoTalkPushDeletedMessageEvent` | Message was deleted                     |
| `message_edited`  | `KakaoTalkPushEditedMessageEvent`  | Message was edited                      |
